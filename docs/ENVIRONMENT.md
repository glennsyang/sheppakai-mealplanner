...
| `AUTH_ALERTS_URL`      | No       | Yes\*\*     | `src/lib/server/notifications/index.ts`                      | Ntfy.sh URL for security/auth push alerts (completed password resets). \*\*Not required to run, but prod delivery needs `fly secrets set AUTH_ALERTS_URL=...`; otherwise it no-ops on a dummy default. |
