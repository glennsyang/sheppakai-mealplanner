	...
	<div class="hidden lg:flex lg:flex-col lg:w-5/12 auth-brand-panel p-12 justify-between text-white">
		...
		<div class="space-y-6">
			...
			<p class="text-primary-100 text-sm leading-relaxed font-light max-w-xs">
				Enter your pantry ingredients and let AI craft dinners around them — then plan your whole week in minutes.
			</p>
		</div>
		...
		>
			...
			<form method="POST" use:enhance class="space-y-5">
				<label class="label">
					...
					/>
					{#if $errors.name}
						<span class="text-error-500 text-xs mt-1 block">{$errors.name}</span>
					{/if}
				</label>
				...
				<label class="label">
					...
					{#if $errors.confirmPassword}
						<span class="text-error-500 text-xs mt-1 block">{$errors.confirmPassword}</span>
					{/if}
				</label>
			...
			<p class="text-center text-sm text-surface-500">
				Already have an account?&ensp;<a href="/login" class="text-primary-600 font-medium hover:underline underline-offset-2">Sign in</a>
			</p>
		</div>
	</div>
</div>
